        ...
        <div class="col-md-9">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('kelola-galeri-video.update', $GaleriVideo->id) }}" autocomplete="off" class="d-flex flex-column gap-20" enctype="multipart/form-data" id="galeri-video-edit" method="post">
                        @csrf
                        @method('put')
                        <div>
                            <label class="form-label fw-bold text-neutral-900" for="judul-video">Nama Video</label>
                            <input class="form-control" id="judul-video" name="judul-video" type="text" value="{{ old('judul-video', $GaleriVideo->judul_video) }}">
                            @error('judul-video')
                        ...
                        <div>
                            ...
                            <input class="form-control" id="link-video" name="link-video" type="text" value="{{ old('link-video', $GaleriVideo->file_video) }}">
                            @error('link-video')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
