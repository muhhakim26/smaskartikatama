...
@section('konten')
    ...
    <form action="{{ route('kelola-galeri-foto.store') }}" enctype="multipart/form-data" id="galeri-foto-create" method="post">
        @csrf
        <div>
            <label for="nama-foto">Nama Foto</label>
            <input id="nama-foto" name="nama-foto" type="text" value="{{ old('nama-foto') }}">
        </div>
        @error('nama-foto')
