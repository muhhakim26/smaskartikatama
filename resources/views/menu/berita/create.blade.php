...
@section('konten')
    ...
    <form action="{{ route('kelola-berita.store') }}" enctype="multipart/form-data" id="berita-create" method="post">
        @csrf
        <div>
            <label for="judul-berita">Judul</label>
            <input id="judul-berita" name="judul-berita" type="text" value="{{ old('judul-berita') }}">
        </div>
        @error('judul-berita')
        ...
        <div>
            ...
            <textarea id="deskripsi-berita" name="deskripsi-berita">{{ old('deskripsi-berita') }}</textarea>
        </div>
